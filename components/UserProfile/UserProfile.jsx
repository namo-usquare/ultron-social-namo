...
const Profile = ({
  ...
  openComponent,
  setContractId,
  // New Props for Wallet Integration
  isWalletConnected,
  walletAddress
}) => {
  ...
  const CHECK_AUTH = async () => {
    try {
      ...
      // Enhanced user data with wallet info
      if (res.status === 200) {
        const userData = {
          ...res.data,
          walletAddress
  ...
  const GET_ALL_POSTS = async () => {
    try {
      ...
      if (res?.status === 200) {
        setAllUserPosts(res?.data.posts);
        updateUserStats(res?.data.posts);
      }
    } catch (err) {
  ...
  // New Function for Stats Update
  const updateUserStats = (posts) => {
    setUserStats({
      posts: posts?.length || 0,
      followers: activeUser?.followers?.length || 0,
      collected: activeUser?.collected?.length || 0
    });
  };

  // Enhanced useEffect with Wallet Integration
  useEffect(() => {
    if (isWalletConnected && activeUser) {
      GET_ALL_POSTS();
    }
  }, [isWalletConnected, activeUser, reCall]);

  // New Wallet Connection Check
  if (!isWalletConnected) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <p className="text-lg font-medium text-base800">
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-base800"></div>
      </div>
    );
  }
      ...
      <div className="flex min-h-screen w-full flex-col bg-white pb-[120px] md:pb-[130px]">
        ...
        <div className="relative">
          <div className="relative w-full bg-base800 h-[232px] md:h-[300px]">
            <img
              alt="cover image"
              className="object-cover object-center"
              src={activeUser?.coverPicture || "sound_letter_s.svg"}
              style={{
                position: "absolute",
                height: "100%",
                width: "100%",
        ...
        <div className="flex gap-2 mt-4 px-4">
          {Object.entries(userStats).map(([key, value]) => (
            <div key={key} className="flex flex-col items-start group cursor-pointer">
              <h3 className="font-title text-title-xs font-medium text-base800 group-hover:text-black md:text-title-m">
                {value}
              </h3>
              <p className="font-base text-base-s font-medium capitalize text-base400 group-hover:text-base500 md:text-base-m">
        ...
        {/* Right Sidebar with Enhanced Props */}
